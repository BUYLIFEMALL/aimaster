...
// 브라우저로는 절대 전달되지 않습니다. (instagram-comment-reply/app/api/instagram/callback/route.ts와 동일 패턴)
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { error } = await supabase.from("dm_accounts").upsert(
      {
        user_id: user.id,
        instagram_user_id: accountInfo.igUserId,

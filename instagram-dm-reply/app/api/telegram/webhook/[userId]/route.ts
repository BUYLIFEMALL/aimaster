 ...
 * URL만 알아낸 제3자가 가짜 승인 요청을 보낼 수 없게 막는다(instagram-comment-reply와 동일 패턴).
 */
export async function POST(request: NextRequest, { params }: { params: { userId: string } }) {
  ...
  const [action, messageId] = String(callback.data).split(":");
  const chatId = String(callback.message?.chat?.id ?? "");
  const tgMessageId = callback.message?.message_id as number | undefined;
  ...
  if (!message) {
    await answerTelegramCallbackQuery({ botToken: telegramLink.bot_token, callbackQueryId: callback.id, text: "메시지를 찾을 수 없습니다." });
  ...
  if (action === "post") {
    const result = await postDmReplyForUser(admin, userId, messageId, message.generated_reply ?? "");
    if (result.error) {
    ...
    statusLine = "\n\n⏸ 답변보류됨 — 웹의 \"DM 검토/발송\" 화면에서 나중에 처리해주세요.";
  } else if (action === "skip") {
    await admin.from("dm_messages").update({ status: "skipped" }).eq("id", messageId);
    toastText = "답변제외했습니다.";
    statusLine = "\n\n❌ 답변제외";
  } else {
    return NextResponse.json({ ok: true });

...
// Storage에 올려 URL로 쓰는 대신 base64 data URI를 콘텐츠에 그대로 심는 기존 관행이 있어서
// (posts/[id]/edit/page.tsx의 replaceBase64WithImageTags·utils/news/imageGenerator.ts 참고),
// 카카오 버전과 달리 파일 업로드/AI 생성 이미지 모두 Storage 업로드 없이 base64로 바로
// 삽입한다 — 이 프로젝트의 기존 데이터 모델과 맞추기 위한 의도적인 차이다.
...
function Toolbar({ editor }: { editor: Editor }) {
  ...
  // 이 팝오버는 상위 <form> 안에 렌더링되므로, 여기서 또 <form action={...}>을 쓰면
  // HTML이 <form> 중첩을 허용하지 않아 브라우저가 조용히 폼을 깨버린다(kakao 버전과
  // 동일한 주의사항) — 그래서 폼 대신 버튼 onClick에서 fetch를 직접 호출한다.
  ...
  return (
    <div className="border-b border-neutral-200">
      ...
      <div className="flex flex-wrap items-center gap-0.5 px-3 py-2">
          ...
          {showAiImageInput && (
            <div className="absolute top-full left-0 z-10 mt-1 min-w-[320px] rounded-lg border border-neutral-200 bg-white p-3 shadow-xl">
              ...
              <textarea
                ...
                placeholder="예: 카페에서 노트북으로 블로그 글을 쓰는 사람"
                rows={2}
                ...
                autoFocus
              />
              {aiError && <p className="mb-2 text-[11px] text-red-600">{aiError}</p>}
              <button
                type="button"
        ...
        {uploadError && <span className="ml-2 text-[11px] text-red-600">{uploadError}</span>}
      </div>
    </div>
  )
}
...
export default function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose max-w-none min-h-[400px] px-4 py-3 focus:outline-none text-sm text-neutral-800 leading-relaxed',
      },
    },
  })

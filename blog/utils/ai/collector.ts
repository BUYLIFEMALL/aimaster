...
// threads/shots의 "AI 콘텐츠 3종 수집 패턴"(docs/PLATFORM_PATTERNS.md §2)을 blog에 이식한 버전.
// 다른 점: threads/shots는 후보 자체가 곧 게시글 본문이지만, blog는 후보가 "주제 + 키워드"
// 소재일 뿐이고 실제 장문 SEO 글 생성은 기존 /api/auto-post 파이프라인이 별도로 담당한다.
...
async function fetchHtml(url: string): Promise<string> {
  const response = await fetch(url, {
    headers: { 'User-Agent': 'Mozilla/5.0 (compatible; AIMaster-Blog/1.0)' },
  })
  if (!response.ok) {
...
const STRUCTURE_SYSTEM_PROMPT = `너는 세계에서 가장 유능한 SEO 블로그 콘텐츠 기획자야. 너의 개인적인 답변은 하지 마.
주어진 원본 자료(뉴스 본문, RSS 피드 항목, 리서치 결과 등)를 바탕으로, 실제로 존재하는 사실만
사용해서 장문 SEO 블로그 글로 발전시킬 만한 주제 후보를 만드세요. 절대 지어내지 마세요.

이 후보는 글 본문이 아니라, 블로그 글쓰기 화면의 "블로그 주제"와 "검색 키워드" 입력칸에
미리 채워 넣을 소재입니다. 아래 규칙을 반드시 지켜서 작성하세요:
1. title은 실제 블로그 글 제목으로 바로 써도 될 만큼 구체적인 한 문장으로 작성하세요 (40자 내외)
2. summary는 이 주제로 글을 쓸 때 다뤄야 할 핵심 각도와 맥락을 2~3문장으로 요약하세요
3. keywords에는 이 주제의 SEO 검색 키워드가 될 만한 단어를 3~6개 담으세요

각 후보는 다음 형식의 JSON 객체입니다:
{
  "title": "위 규칙을 지킨 블로그 주제 제목",
  "summary": "이 주제로 글을 쓸 때 다뤄야 할 핵심 내용 요약 (2~3문장)",
...
export async function structureBlogCandidates(params: {
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        {
          role: 'user',
          content: `아래 원본 자료로 블로그 주제 후보를 최대 ${maxItems}개 만들어주세요.\n\n${rawText}`,
        },
      ],
  ...
  const candidates = (parsed.candidates ?? []).filter((c) => c?.title)
  if (candidates.length === 0) {
    throw new Error('생성된 블로그 주제 후보가 없습니다.')
  }

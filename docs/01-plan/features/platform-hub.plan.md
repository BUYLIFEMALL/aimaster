...
> **Summary**: 앞으로 만들어질 여러 AI 웹프로그램들을 aimaster 하나의 코드베이스(모노레포/서브폴더 구조) 안에서 함께 개발하고, 기존 계정·구독·결제·관리자 시스템을 공유하며 통합 운영한다.
>
> **Project**: ai-master
> **Version**: 0.1.0
> **Author**: buylifemall@gmail.com
> **Date**: 2026-07-22
> **Status**: Approved (2026-07-22 — 아키텍처 결정: 프로그램별 독립 폴더 + 단일 Supabase DB 공유)
...
현재 aimaster는 마케팅 자동화 프로그램에 대한 "판매(구독/결제) 사이트" 역할만 하고, 실제 AI 웹프로그램 자체는 프로그램마다 별도로 존재하는 것을 전제로 설계되어 있었다.
앞으로 새로 만들어지는 AI 웹프로그램들은 완전히 흩어진 별도 사이트가 아니라, **aimaster와 같은 git 저장소(레포) 안에 프로그램별 독립 폴더**로 두고 각자 자체 Next.js 프로젝트(자체 `package.json`/배포)로 개발하되, **계정/구독(subscriptions)/결제(payment_records)/등급(member_grades)/관리자(admin) 시스템은 aimaster와 동일한 Supabase 프로젝트(단일 DB)를 공유**해서 재사용하는 통합 플랫폼(허브)으로 운영한다.

> **결정 배경 (2026-07-22)**: 최초 초안에서는 "하나의 Next.js 코드베이스 안에 라우트로 추가"(`app/(dashboard)/apps/[slug]/`)를 검토했으나, 실제로는 프로그램마다 독립적으로 개발/배포하는 편이 낫다고 판단하여 "폴더는 분리, DB는 공유"로 확정했다. 첫 사례인 `threads/`(Threads 자동 포스팅)가 이 패턴으로 이미 구현되어 있다.

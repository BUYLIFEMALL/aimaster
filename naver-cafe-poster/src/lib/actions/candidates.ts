...
import {
  ...
  type CafeCandidateDraft,
} from "@/lib/ai/collector";
import type { CandidateSourceType } from "@/types/post";
...
async function insertCandidates(
  ...
  const now = Date.now();
  const { error } = await supabase.from("ncafe_candidates").insert(
    drafts.map((d, i) => ({
      user_id: userId,
...
/**
 * 방식 1: HTTP — 주어진 URL이 특정 게시글이면 그 글로 게시글 후보 1건을,
 * 여러 게시글이 나열된 카테고리/목록 페이지면 그중 무작위 5건을 골라 각각 후보를 생성한다.
 */
...
/** 방식 2: RSS(NewsBlur) — 연결된 NewsBlur 계정에서 선택한 구독 피드의 최근 글로 게시글 후보를 생성한다. */
...
/** 방식 3: Perplexity — 시드 주제로 현재 트렌딩 앵글을 찾아 게시글 후보를 생성한다. */
...
export async function deleteCandidateAction(formData: FormData) {
  ...
  await supabase.from("ncafe_candidates").delete().eq("user_id", user.id).eq("id", id);

...
// threads/shots의 "AI 콘텐츠 3종 수집" 패턴을 그대로 재사용한다(docs/PLATFORM_PATTERNS.md §2,
// threads/src/lib/ai/collector.ts와 구조 100% 동일 — HTTP/RSS 스크래핑·NewsBlur 연동·
// Perplexity 검색 로직은 도메인 무관이라 그대로 가져오고, 구조화 프롬프트만 카페 게시글
// 톤(존댓말, 글자수 제한 없음)에 맞게 바꿨다.

export interface CafeCandidateDraft {
  title: string;
  content: string;
  keywords: string[];
}
...
async function fetchHtml(url: string): Promise<string> {
  const response = await fetch(url, {
    headers: { "User-Agent": "Mozilla/5.0 (compatible; AIMaster-NaverCafePoster/1.0)" },
  });
  if (!response.ok) {
...
export async function searchPerplexityTrending(topic: string, apiKey: string): Promise<string> {
  if (!apiKey) {
    throw new Error("Perplexity API 키가 없습니다. 설정 페이지에서 본인의 Perplexity API 키를 등록해주세요.");
  }
...
// Threads와 달리 글자수 제한이 없고, 카페 회원들에게 정보를 전달하는 정중한 존댓말 톤을 쓴다
// (cafeGenerator.ts의 CAFE_SYSTEM_PROMPT와 동일한 톤 규칙 — 원본 자료 기반이라는 점만 다르다).
const STRUCTURE_SYSTEM_PROMPT = `너는 네이버 카페 운영 경험이 많은 카페 매니저야. 너의 개인적인 답변은 하지 마.
주어진 원본 자료(뉴스 본문, RSS 피드 항목, 리서치 결과 등)를 바탕으로 실제로 존재하는 사실만
사용해서, 카페 게시판에 바로 올려도 될 만큼 완성도 있는 게시글 후보를 만드세요. 절대 지어내지 마세요.

각 후보는 아래 규칙을 반드시 지켜서 작성하세요 (요약문이 아니라 실제 게시글입니다):
1. title(제목)은 20자 이내로 작성하세요
2. content(본문)는 정중한 존댓말(-습니다/-해요체)로 작성하고, 글자수 제한은 없습니다
3. 1~2문장마다 문단을 끊고, 문단과 문단 사이에는 반드시 줄바꿈을 두 번(JSON 문자열 안에서 \n\n) 넣어서 시각적으로 나누어 주세요. 하나의 긴 문단으로 이어 쓰지 마세요
4. 카페 회원들에게 도움이 되는 정보 전달에 집중하고, 과도한 광고 느낌은 피하세요
5. keywords에 담을 단어는 본문에도 자연스럽게 녹여 넣으세요 (해시태그 나열 금지)

각 후보는 다음 형식의 JSON 객체입니다:
{
  "title": "위 규칙을 지킨 제목 (20자 이내)",
  "content": "위 규칙을 지킨 카페 게시글 본문",
...
export async function structureCafeCandidates(params: {
  rawText: string;
  maxItems: number;
  apiKey: string;
  ...
  let parsed: { candidates?: CafeCandidateDraft[] };
  try {
  ...
  const candidates = (parsed.candidates ?? [])
    ...
    .map((c) => ({ ...c, content: ensureParagraphBreaks(c.content) }));
  if (candidates.length === 0) {
    throw new Error("생성된 게시글 후보가 없습니다.");
  }

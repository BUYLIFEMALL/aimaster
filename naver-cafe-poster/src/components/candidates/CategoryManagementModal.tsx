...
/**
 * blog(BLOG 원문생성 자동화)의 CategoryManagementModal 패턴을 이식(2026-09-16, 사용자 요청
 * "이방식으로 카테고리를 구현하는게 편할것 같다 공간도 적게 잡아먹고") — 기존에는 카테고리
 * 목록/추가폼이 페이지에 항상 펼쳐져 있어 공간을 많이 차지했는데, 팝업 모달로 옮기고
 * 평소에는 카테고리 이름 칩만 보여주도록 CategoryManager.tsx를 함께 바꿨다. 실제 추가/수정/
 * 삭제/순서변경 로직(서버 액션)은 그대로 재사용 — UI 셸만 모달로 감쌌다.
 */
      ...
      <div className="flex max-h-[85vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-2xl">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-6">
          <p className="text-xs text-neutral-500">
            카테고리를 만들어두면 글감 수집 시 어느 카테고리에 넣을지 고를 수 있고, "수집된
            게시글 후보" 목록에서 카테고리별로 필터링·이동해서 관리할 수 있습니다. 카테고리를
            삭제해도 그 카테고리로 분류돼 있던 후보는 삭제되지 않고 "카테고리 없음"으로
            돌아갑니다.
          </p>

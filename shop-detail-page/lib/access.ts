...
const THIS_PROGRAM_SLUG = "shop-detail-page";
...
/**
 * 로그인 + "shop-detail-page" 프로그램 이용 권한을 함께 확인한다.
...
> {
  ...
  return { allowed: false, error: "상세페이지 자동화(15p) 이용 권한이 없습니다. 구독 후 이용해주세요.", status: 403 };
}

/**
 * Gemini 분석/이미지생성처럼 실제 비용이 발생하는 동작 1건을 usage_logs에 기록한다.
 * RLS insert 정책상 service role(admin client)로만 기록 가능하다.
 * 텔레메트리 성격이므로 기록 실패가 본 기능을 막아서는 안 된다.
 */

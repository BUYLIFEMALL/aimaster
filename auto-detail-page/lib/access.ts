...
// 이 앱(auto-detail-page)은 AIMaster와 같은 Supabase 프로젝트를 공유한다.
// 로그인 여부만으로는 부족하고, AIMaster의 programs/subscriptions/user_program_access
// 테이블을 기준으로 "이 프로그램을 실제로 이용할 권한이 있는지"까지 확인해야 한다.
// insta_auto_poster/src/lib/access.ts, threads/shots와 동일한 패턴.
//
...
const THIS_PROGRAM_SLUG = "auto-detail-page";
...
/**
 * 로그인 + "auto-detail-page" 프로그램 이용 권한을 함께 확인한다.
...
/**
 * Claude/이미지 생성 API 호출처럼 실제 비용이 발생하는 동작 1건을 usage_logs에 기록한다.
 ...
 * 받아 res.json() 파싱에 실패하는 문제가 있어 분리했다. threads/blog/insta_auto_poster와 동일 패턴.)
 */
...
> {
  ...
  return { allowed: false, error: "상세페이지 자동생성 이용 권한이 없습니다. 구독 후 이용해주세요.", status: 403 };
}

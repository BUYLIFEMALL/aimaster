...
type Job = { id: string; name: string; size: number; file?: File; status: "ready" | "queued" | "processing" | "done" | "error"; progress: number; outputSize?: number; error?: string; downloadUrl?: string };
const MAX_FILES = 20;
...
export default function ConverterWorkspace() {
  ...
  async function start() {
    ...
    setJobs((current) => current.map((job) => job.status === "ready" ? { ...job, status: "queued" } : job));
    const body = new FormData(); body.set("fps", String(fps)); body.set("width", String(width));
    ready.forEach((job) => body.append("files", job.file!, job.name));
    try {
      ...
      for (const serverJob of payload.jobs as Array<{ jobId: string; name: string }>) {
        const local = ready.find((job) => job.name === serverJob.name);
        if (!local) continue;
        setJobs((current) => current.map((job) => job.id === local.id ? { ...job, id: serverJob.jobId, status: "processing", progress: 1 } : job));
    ...
    } catch (error) { setJobs((current) => current.map((job) => ready.some((item) => item.id === job.id) ? { ...job, status: "error", error: error instanceof Error ? error.message : "변환 요청 실패" } : job)); }
  }

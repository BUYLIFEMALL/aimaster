import Link from "next/link";

const MAIN_SITE_URL = process.env.NEXT_PUBLIC_MAIN_SITE_URL ?? "https://www.buylife.xyz";

// 회원가입은 AIMaster에서만 받는다 — 모든 AI 프로그램은 AIMaster 계정/구독 권한을 공유한다.
export default function SignupPage() {
  return (
    <main className="min-h-screen bg-slate-950">
      <div className="mx-auto max-w-sm px-4 py-24 text-center">
        <Link href="/" className="text-lg font-black tracking-tight text-white">
          Video<span className="text-amber-400">ToGIF</span>
        </Link>
        <h2 className="mb-2 mt-6 text-lg font-bold text-white">회원가입은 AIMaster에서 진행됩니다</h2>
        <p className="mb-6 text-sm leading-relaxed text-slate-400">
          한 번 가입하면 VideoToGIF는 물론, AIMaster의 다른 모든 프로그램도
          <br />
          같은 계정으로 바로 이용할 수 있어요.
        </p>
        <a
          href={`${MAIN_SITE_URL}/register`}
          className="inline-block w-full rounded-xl bg-amber-400 px-4 py-3 text-sm font-bold text-slate-950 transition-colors hover:bg-amber-300"
        >
          AIMaster 회원가입 하러가기
        </a>
        <p className="mt-4 text-center text-sm text-slate-400">
          이미 계정이 있으신가요?{" "}
          <Link href="/login" className="font-medium text-amber-400 underline">
            로그인
          </Link>
        </p>
      </div>
    </main>
  );
}

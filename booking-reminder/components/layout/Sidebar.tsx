...
// 예약(취소)방지 리마인드는 "예약 등록 → 리마인드 규칙 설정 → 자동 발송"이 한 페이지
// (/reservations) 안에서 이어지는 단일 핵심 흐름이라, 스텝 1개짜리 스텝퍼로 보여준다.
// 발송 계정(이메일/문자·카카오/텔레그램) 연동은 이 흐름의 일부가 아니라 별도 유틸리티라 분리.
const FLOW_STEPS = [
  {
    step: 1,
    href: "/reservations",
    icon: "📅",
    label: "예약 관리",
    description: "예약 등록 · 리마인드 규칙 설정 · 발송 현황",
  },
];

const UTILITY_ITEMS = [{ href: "/settings", icon: "📨", label: "플랫폼연동" }];
          ...
          {FLOW_STEPS.map((item, idx) => {
            ...
            return (
              <Link key={item.href} href={item.href} className="group relative flex gap-3 pb-1">
                ...
                {/* 라벨 + 설명 */}
                <div className={`min-w-0 flex-1 rounded-lg px-2 py-1.5 ${isActive ? "bg-blue-50" : "group-hover:bg-neutral-50"}`}>
                  <p className={`text-sm font-bold ${isActive ? "text-blue-700" : "text-neutral-800"}`}>
            ...
            return (
              <Link
                ...
                className={`block rounded-lg px-3 py-2 text-sm font-medium ${
                  isActive ? "bg-blue-50 text-blue-700" : "text-neutral-600 hover:bg-neutral-100 hover:text-neutral-900"
                }`}
              >
                {item.icon} {item.label}
              </Link>

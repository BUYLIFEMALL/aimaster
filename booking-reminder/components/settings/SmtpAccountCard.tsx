...
export function SmtpAccountCard({ account }: { account: SmtpAccountData }) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
      ...
      {account.from_name && <p className="text-xs text-gray-400 mt-0.5">발신자 표시: {account.from_name}</p>}

      {testResult && (
        <p className={`mt-2 text-xs ${testResult.ok ? "text-green-600" : "text-red-600"}`}>{testResult.message}</p>
      )}
      ...
      {isEditing && (
        <form onSubmit={handleSaveEdit} className="mt-4 space-y-3 border-t border-gray-100 pt-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">계정 별칭</label>
              <input name="label" required defaultValue={account.label} className="input" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">발신자 표시 이름 (선택)</label>
              <input name="fromName" defaultValue={account.from_name ?? ""} className="input" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">SMTP 호스트</label>
              <input name="smtpHost" required defaultValue={account.smtp_host} className="input" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">포트</label>
              <input name="smtpPort" type="number" required defaultValue={account.smtp_port} className="input" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">로그인 계정 (이메일)</label>
            <input name="smtpUser" type="email" required defaultValue={account.smtp_user} className="input" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">비밀번호 (앱 비밀번호)</label>
            <input
              name="smtpPassword"
              type="password"

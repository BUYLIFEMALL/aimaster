...
// 경쟁사 키워드 분석은 "키워드 등록+분석 실행 → 분석 결과 확인"으로 이어지는 2단계 핵심
// 흐름이라, 사이드바도 그 순서를 위→아래 스텝퍼로 보여준다. "내 경쟁사"는 특정 키워드 하나가
// 아니라 지금까지 등장한 경쟁사를 키워드 전체에 걸쳐 모아보는 부가 기능이라 순서에 넣지 않고
// 스텝퍼 아래 별도 자리에 둔다. API 키 설정은 이 프로그램의 순차 흐름이 아닌 유틸리티라 맨 아래로 분리.
const FLOW_STEPS = [
  {
    step: 1,
    href: "/keywords",
    ...
    <aside className="flex w-full flex-col border-b border-neutral-200 bg-white p-4 md:h-full md:w-64 md:justify-between md:border-b-0 md:border-r">
      <div>
        <div className="mb-4 md:mb-6">
          <div className="px-2 text-lg font-semibold text-neutral-900">경쟁사 키워드분석 자동화</div>
          <a
          ...
          {SECONDARY_ITEMS.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`rounded-lg px-3 py-2 ${isActive ? "bg-sky-50" : "hover:bg-neutral-50"}`}
              >
                <p className={`text-sm font-bold ${isActive ? "text-sky-700" : "text-neutral-800"}`}>
                  {item.icon} {item.label}
                </p>

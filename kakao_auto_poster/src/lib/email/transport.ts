...
// trending-product-finder/lib/email/transport.ts와 동일한 패턴 — 리포트 이메일 알림은
// 운영자 공용 SMTP가 아니라 회원 각자가 설정 페이지에서 등록한 SMTP 계정을 쓴다(BYOK).
// 공용 `user_smtp_accounts` 테이블(stepmail 등이 이미 만들어둔 것)을 그대로 재사용해서,
// 다른 프로그램에서 이미 등록한 계정이 있으면 여기서도 바로 재사용된다.

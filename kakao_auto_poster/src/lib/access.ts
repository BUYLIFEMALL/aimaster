...
/**
 * 로그인 + "kakao-auto-posting" 프로그램 이용 권한을 함께 확인한다.
...
/**
 * OpenAI/Gemini/Perplexity 호출처럼 실제 비용이 발생하는 동작 1건을 usage_logs에 기록한다.
...
/**
 * requireProgramAccess()와 동일한 판정 로직이지만, API route handler(크론 등)에서
...
> {
  ...
  return { allowed: false, error: "카카오톡 정보 콘텐츠 자동화 이용 권한이 없습니다. 구독 후 이용해주세요.", status: 403 };
}

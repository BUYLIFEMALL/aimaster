...
// SOLAPI 공식 Node.js SDK(solapi 패키지)를 감싼 얇은 래퍼 — trending-product-finder/
// crm-google-form의 lib/solapi/client.ts와 동일한 코드를 그대로 재사용한다. HMAC-SHA256
// 서명/인증은 SDK가 전부 처리한다(직접 구현하지 않음, https://solapi.com/developers 참고).
//
 ...
 * 아닌 수신자에게도 발송할 수 있다(정보성 메시지). Phase 2에서는 쓰지 않고, 나중에 필요하면
 * crm-google-form/booking-reminder와 동일하게 확장할 수 있도록 남겨둔다.
 */
 ...
 * 브랜드 메시지(자유형)로 대체 발송하지만, 요청 필드 자체는 바뀌지 않는다. 이 프로젝트에서는
 * 리포트 요약을 카카오 채널 구독자에게 발송하는 데 쓴다.
 */

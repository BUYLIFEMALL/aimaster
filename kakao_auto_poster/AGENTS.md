...
- **Phase 12 카카오톡 공유(원하는 채팅방에 공유하기) — 배포 전 반드시 필요한 콘솔 설정**:
  코드는 완성됐지만 아직 Vercel에 배포하지 않았고, 아래 작업 없이는 배포해도 동작하지 않는다.
  1. 카카오 개발자 콘솔에서 `KAKAO_REST_API_KEY`가 속한 앱의 "JavaScript 키"를 확인해
     Vercel 환경변수 `NEXT_PUBLIC_KAKAO_JS_KEY`로 등록.
  2. 그 앱의 **"플랫폼 키 > JavaScript SDK 도메인"** 과 **"제품 링크 관리 > 웹 도메인"**
     두 곳 모두에 `https://kakaoautoposter.vercel.app`(실제 프로덕션 도메인) 등록 — mbti-character
     트러블슈팅에서 확인한 대로, 이 중 하나라도 빠지면 카드는 보여도 클릭 시 이동이 안 되거나
     PC에서 "모바일에서 확인해주세요"만 뜨고 클릭이 막힌다.
  3. 배포 후 실제 리포트 상세 페이지에서 "💬 카카오톡 공유" 버튼으로 실제 채팅방에 보내고,
     카드 클릭 시 `/share/[token]` 페이지가 로그인 없이 정상적으로 열리는지 실기기로 확인할 것.
  4. `/api/og` 동적 카드 이미지가 실제 카카오톡 미리보기에 정상적으로 뜨는지도 함께 확인.

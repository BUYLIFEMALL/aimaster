...
- **새 서브프로젝트를 시작하거나 기존 서브프로젝트를 이어서 작업할 때는, 코드를 만들기 전에
  ...
  그 지침을 그대로 따른다.** 특히 "Platform-hub 구조"·"서브프로젝트 작업물은 반드시 해당
  서브프로젝트 폴더 안에서 관리한다"·"멀티테넌시 원칙" 세 섹션은 예외 없이 적용한다
  (2026-08-24: 사용자가 별개의 로컬 클론(`D:\Antigravity\AIMaster`)에서 최신 작업이 안 보인다고
  여러 차례 문의한 사례가 있었다 — 실제로는 그 클론이 22커밋 뒤처져 있던(pull 안 한) 것이
  원인이었다. "여러 로컬 클론이 존재할 수 있다"는 원칙을 이 파일이 이미 명시하고 있었으니,
  새 프로젝트 작업 시작 전 이 문서를 스스로 다시 확인하는 습관이 이런 혼선을 줄인다).

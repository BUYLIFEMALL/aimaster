...
export async function connectTelegramAction(
  ...
  try {
    await sendTelegramMessage({
      ...
      text: "✅ Threads 댓글자동화 알림 연동이 완료됐어요. 쓰레드 계정 연결이 끊어지면 여기로 알려드리고, 새 댓글이 오면 여기서 바로 답변승인/답변보류/답변제외를 선택할 수 있어요.",
    });
  } catch {
  ...
  try {
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? "https://threads-comment-reply.vercel.app";

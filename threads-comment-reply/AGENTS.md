...
7. **실제 쓰레드 댓글 게시(`postReplyAction`)** — 공개적으로 남는 되돌리기 어려운 행동이다.
   ...
   게시하는 코드를 추가하지 말 것. **instagram-comment-reply와 마찬가지로 (선택) 자동 게시
   토글은 Phase 1에 아예 만들지 않는다** — Meta 정책 §5.2.2(d)가 "정형화되지 않은, 사전 승인된"
   메시지만 허용한다고 더 엄격하게 명시하고 있어 안전 마진을 더 둔다(추가하려면 별도로 신중히
   검토).

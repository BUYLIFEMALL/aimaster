...
export default async function HistoryPage() {
  ...
  const [{ data: posted }, { count: postedTotal }, { count: postedThisWeek }, { count: pendingCount }] =
    await Promise.all([
      supabase
        .from("th_comments")
        .select("id, post_id, author_username, comment_text, generated_reply, replied_at")
        .eq("user_id", user.id)
        .eq("status", "posted")
  ...
  const mediaIds = Array.from(new Set((posted ?? []).map((c) => c.post_id)));
          ...
          {posted.map((c) => (
            <div key={c.id} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
              <div className="mb-2 flex items-center justify-between">
                <p className="text-xs font-semibold text-gray-400">🧵 {captionByMediaId.get(c.post_id) || "(삭제된 게시물)"}</p>

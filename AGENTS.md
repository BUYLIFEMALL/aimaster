...
- 사용자 요청 작업은 기능 변경, 검수, 커밋, 푸시, 배포를 한꺼번에 묶지 않고 각 단계를 순서대로 하나씩 처리한다. 각 단계가 끝나면 결과를 보고한다. 단, 사용자가 작업 완료 후 커밋·푸시·배포까지 자동 진행하라고 명시한 경우에는 해당 순서를 자동 실행하되 각 단계 결과를 기록한다.

- 사소한 구현 방식 선택은 재질문하지 않고 합리적으로 판단해 진행한다. 기존 코드 구조/디자인/
  명명 규칙을 우선 따른다.
- 파일 생성, 코드 수정, 패키지 설치, 빌드/테스트/오류 수정까지 중간 확인 없이 연속 수행한다.
- **비밀번호·API 키 필요, 비용 발생, 실제 데이터 삭제, 그리고 원래는 배포/Git Push도 질문
  대상이었으나 — 2026-09-10(kakao_auto_poster)과 2026-09-16(naver-cafe-poster) 두 번
  독립적으로 "앞으로 묻지 말고 커밋·푸시·배포를 한 세트로 진행해달라"는 지시가 있어서, 지금은
  검증까지 끝난 일반적인 코드/기능 변경은 빌드 확인 → 커밋 → 푸시 → 배포 → 라이브 확인까지
  묻지 않고 자동으로 진행하는 게 기본값이다.** 여전히 확인이 필요한 것: 진짜 파괴적인 작업
  (데이터 삭제, force-push, 커밋 안 된 작업 버리기), 환경변수/DB 스키마 변경(스키마 변경은
  1회 확인 후 진행, env var 추가도 1회 확인).
- **사용자가 여러 요청을 연달아 보내도 섞어서 한꺼번에 처리하지 않는다.** 각 요청을 하나의
  작업 단위로 구분해서 순서대로 처리하고 보고한다(빌드/커밋/배포 같은 기계적 사이클은 묶어도
  되지만, 사용자가 요청한 항목 단위는 흐려지지 않게 유지).
...
- 작업 완료 후에는 변경사항·테스트 결과·남은 문제만 간결하게 정리해서 보고한다.

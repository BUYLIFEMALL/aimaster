...
### 2. 필수 검증
1. **데이터 수집일** ≤ 오늘(분석 시점)인지 확인 — 매물을 수집한 **당일**에 바로 분석하는 것이
   이 서비스의 정상적인 실시간 처리 흐름이므로, 데이터 수집일이 오늘과 같은 날짜인 것은
   오류가 아니다 (수집일이 분석일보다 미래일 때만 비정상으로 판단).
2. \`건축년도 ≤ 데이터 수집일 연도\` 인지 확인
3. 면적·가격 항목은 **양수**인지 검증 (단, 값이 "-"로 제공되어 원래 없는 항목은 검증 대상에서 제외)

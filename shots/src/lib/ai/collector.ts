...
export interface ShortsCandidateDraft {
  title: string;
  hook: string;
  content: string;
  keywords: string[];
...
async function fetchHtml(url: string): Promise<string> {
  const response = await fetch(url, {
    headers: { "User-Agent": "Mozilla/5.0 (compatible; AIMaster-Shots/1.0)" },
  });
  if (!response.ok) {
...
const STRUCTURE_SYSTEM_PROMPT = `당신은 유튜브 쇼츠 기획자입니다. 주어진 원본 자료(뉴스 본문, RSS 피드 항목, 리서치 결과 등)를
바탕으로 실제로 존재하는 사실만 사용해서 유튜브 쇼츠 영상 주제 후보를 만드세요. 절대 지어내지 마세요.

각 후보는 다음 형식의 JSON 객체입니다:
{
  "title": "영상 제목 (20자 이내, 클릭을 유도하는 후킹 문구)",
  "hook": "영상 시작 3초 안에 쓸 훅 문장 1개",
  "content": "쇼츠 대본 초안 (3~6문장, 짧고 임팩트 있게)",
...
export async function structureCandidates(params: {
  rawText: string;
  maxItems: number;
  apiKey: string;
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `아래 원본 자료로 쇼츠 주제 후보를 최대 ${maxItems}개 만들어주세요.\n\n${rawText}`,
        },
      ],
  ...
  let parsed: { candidates?: ShortsCandidateDraft[] };
  try {
  ...
  if (candidates.length === 0) {
    throw new Error("생성된 쇼츠 주제 후보가 없습니다.");
  }

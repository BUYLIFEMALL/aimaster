...
AIMaster 계정으로 로그인해야 이용할 수 있는 **AI 타로 리딩 웹앱**. 질문을 입력하고 11종
스프레드(원카드/3카드 과거현재미래/3카드 궁합/5카드 심층분석/켈틱크로스 10카드/말굽
7카드/관계 6카드/커리어 6카드/한 해 운세 12카드/생명의 나무 10카드/만다라 9카드) 중
하나를 골라 카드를 뽑으면,
...
심층분석, 가장 정통적인 10카드 켈틱 크로스, 조언 중심의 7카드 말굽(Horseshoe), 나와
상대방을 함께 깊게 보는 6카드 관계(Relationship), 진로 고민에 특화된 6카드 커리어
(Career), 1월부터 12월까지 한 달에 카드 한 장씩 보는 12카드 한 해 운세(Year Ahead),
카발라 세피로트에 기반한 10카드 생명의 나무(Tree of Life), 그리고 삶의 여러 영역을
한 번에 점검하는 9카드 만다라 라이프 밸런스까지 총 11종 스프레드를 추가했다(사용자
요청, 2026-09-19: "실제 타로 세계의 다른 유명 스프레드들도 하나씩 완성해나가보자" —
켈틱 크로스 다음으로 널리 쓰이는 정통 스프레드부터 순서대로 추가하는 중). 각 스프레드는
`positions`(자리 순서)·
`positionLabels`(자리별 한글 라벨)·`positionDescriptions`(자리별 설명)를 자체적으로
갖고 있어, 카드 뽑기(`drawCards()`)와 결과 화면(`ResultInteractive`)이 스프레드
종류에 관계없이 이 설정 하나로 동작한다. 카드 수가 많은 스프레드는 한 번에 해석해야
할 카드가 많아 `/api/generate-reading`의 시스템 프롬프트를 스프레드별로 따로 두고,
OpenAI 응답 토큰 한도(`max_completion_tokens`)도 카드 6장 이상이면 2000 → 3000,
10장 이상이면 3500으로 늘려 흐름이 끊기지 않게 했다.

관계(Relationship) 스프레드는 기존 "3카드 궁합"(나의 마음/상대방의 마음/우리의 미래)의
심화 버전이다 — 나와 상대방 각자의 역할, 두 사람의 연결고리, 관계의 강점, 함께 풀어야
할 과제, 나아갈 방향까지 6개 자리로 더 입체적으로 본다. 특정 관계 유형(연인/친구/가족
등)이나 성별을 전제하지 않고 중립적으로 해석하도록 시스템 프롬프트에 명시했다.

커리어(Career) 스프레드는 켈틱 크로스·말굽처럼 하나로 고정된 역사적 명칭의 스프레드는
아니고(실제 타로 업계에서도 "커리어 스프레드"는 사이트마다 자리 구성이 조금씩 다르다),
실용적인 진로 상담에 흔히 쓰이는 "현재 상황 → 강점 → 걸림돌 → 기회 → 조언 → 결과"
6단계 구조로 만들었다. 시스템 프롬프트에도 확정적인 취업·이직·창업 성공을 단언하지
말고 참고할 방향성으로만 제시하도록 명시했다.

한 해 운세(Year Ahead) 스프레드는 12개월 각각을 별도 자리로 관리한다 — 자리 키/라벨을
`lib/deck.ts`에 매달 손으로 12줄씩 적는 대신 `YEAR_AHEAD_MONTH_KEYS`/
`YEAR_AHEAD_MONTH_NAMES` 배열에서 한 번에 만들어 쓴다(새 스프레드 추가 시 반복 패턴이면
이렇게 프로그래밍적으로 생성해도 된다는 선례). 카드 12장을 한 달씩 전부 개별 문단으로
풀면 응답이 지나치게 길어지므로, AI 해석은 "전체 흐름 요약 1~2문단 + 분기별(1~3월/
4~6월/7~9월/10~12월) 4문단"으로 압축해서 작성하도록 시스템 프롬프트에 명시했다.

생명의 나무(Tree of Life) 스프레드는 유대 신비주의 카발라의 "세피로트(Sephirot)" 10개
자리를 그대로 타로에 적용한 정통 신비주의 스프레드다(황금새벽회 전통 이후 여러 타로
유파가 실제로 쓰는 방식). Keter(영적 목표)부터 Malkuth(현실)까지 위에서 아래로
내려오는 순서를 그대로 자리 순서로 삼았고, 각 자리 라벨에 히브리어 세피로트 이름을
괄호로 함께 표기해 정통성을 살리면서도 한글 설명으로 이해하기 쉽게 했다.

만다라(Mandala) 스프레드는 원래 계획에는 "별/만다라"로 함께 묶여 있었는데, 실제로
"별 스프레드"와 "만다라 스프레드" 둘 다 타로 업계에서 자리 구성이 표준화돼 있지 않고
서로 개념이 겹쳐서(둘 다 원형/방사형으로 삶의 여러 면을 살피는 스프레드), 하나로
합쳐 실용적인 9카드 "라이프 밸런스" 스프레드로 만들었다 — 중앙의 "지금 나의 핵심
테마" 카드를 중심으로 사랑·일·재정·건강·가족·자기계발·영성·여가 8개 영역을 배치했다.
단일 질문을 깊게 파는 다른 스프레드들과 달리, 이 스프레드는 "삶 전체를 한 번에
점검"하는 용도라 AI 해석도 영역마다 2~3문장으로 간결하게 정리하도록 시스템 프롬프트에
명시했다(9장을 전부 긴 문단으로 풀면 지나치게 길어지기 때문).

**남은 추가 예정 스프레드(순서대로 진행 예정)**: 예/아니오(Yes-No) 스프레드. 새
스프레드를 추가할 때는 이 파일의 "말굽 스프레드"/"관계 스프레드"/"커리어 스프레드"/
"한 해 운세 스프레드"/"생명의 나무 스프레드"/"만다라 스프레드" 추가 방식(①`lib/deck.ts`에
`SpreadType`/필요한 `SpreadPosition`/`SPREAD_CONFIGS` 항목 추가 ②`app/api/generate-
reading/route.ts`에 전용 시스템 프롬프트 추가 ③카드 수가 6장 이상이면 토큰 한도
확인)을 그대로 따르면 된다.
...
카드 이미지/글자 크기도 `/result`(최초 생성 화면)와 동일한 그리드 로직과 폰트 크기로
맞췄다(2026-09-19, 사용자 지적: "보관함 이미지/글자가 너무 작다"). 각 카드 썸네일을
클릭하면 `/result`의 "클릭하여 확대 보기"와 동일한 라이트박스 모달로 원본 이미지 크기를
크게 볼 수 있다(2026-09-19 추가). 카드 그리드는 카드가 몇 장이든(5장·7장·10장 스프레드
포함) **한 줄에 최대 3장까지만** 보여주고 그 이상은 다음 줄로 넘긴다(5장이면 3+2, 10장이면
3+3+3+1) — 예전엔 5장 이상일 때 한 줄에 5열까지 욱여넣어서 카드 폭이 좁아지고 해설 글자가
세로로 잘려 보이는 문제가 있었다(2026-09-19, 사용자 지적: "한 줄에 3장씩이 적당한 것
같아"). `/result`와 `/history` 양쪽 다 동일한 규칙(`ResultInteractive.tsx`/
`HistoryList.tsx`의 `gridColsClass`)을 쓴다.
...
| 15 | `/history` 카테고리 필터 + 최신 항목 자동 펼침 + 이미지 확대 라이트박스 | ✅ |
| 16 | 말굽(Horseshoe) 7카드 스프레드 추가 | ✅ |
| 17 | 카드 그리드 한 줄 최대 3장으로 통일(5·7·10카드 스프레드 가독성 개선) | ✅ |
| 18 | 관계(Relationship) 6카드 스프레드 추가 | ✅ |
| 19 | 커리어(Career) 6카드 스프레드 추가 | ✅ |
| 20 | 한 해 운세(Year Ahead) 12카드 스프레드 추가 | ✅ |
| 21 | 생명의 나무(Tree of Life) 10카드 스프레드 추가 | ✅ |
| 22 | 만다라 라이프 밸런스 9카드 스프레드 추가 | ✅ |
| 23 | 예-아니오(Yes-No) 스프레드 추가 | ⬜ 남은 작업(순서대로 진행 예정) |

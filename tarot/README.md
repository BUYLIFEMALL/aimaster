...
AIMaster 계정으로 로그인해야 이용할 수 있는 **AI 타로 리딩 웹앱**. 질문을 입력하고 12종
스프레드(원카드/3카드 과거현재미래/3카드 궁합/5카드 심층분석/켈틱크로스 10카드/말굽
7카드/관계 6카드/커리어 6카드/한 해 운세 12카드/생명의 나무 10카드/만다라 9카드/
예·아니오 3카드) 중 하나를 골라 카드를 뽑으면,
AI가 그린 카드 일러스트(Gemini, 12종 화풍 선택 가능)와 AI가 풀어주는 종합 해석(OpenAI, 모델
...
카발라 세피로트에 기반한 10카드 생명의 나무(Tree of Life), 삶의 여러 영역을 한 번에
점검하는 9카드 만다라 라이프 밸런스, 그리고 카드 3장의 정/역방향 다수결로 명쾌한
답을 주는 예·아니오(Yes-No) 스프레드까지 총 12종 스프레드를 추가했다(사용자 요청,
2026-09-19: "실제 타로 세계의 다른 유명 스프레드들도 하나씩 완성해나가보자" — 켈틱
크로스 다음으로 널리 쓰이는 정통 스프레드부터 순서대로 진행해, 계획했던 스프레드를
전부 완성했다). 각 스프레드는
`positions`(자리 순서)·
...
예·아니오(Yes-No) 스프레드는 카드 3장을 뽑아 **정방향/역방향 개수의 다수결**로 판정하는
전통적인 방식이다(정방향 2장 이상 → "예"에 가까움, 역방향 2장 이상 → "아니오"에
가까움). AI 해석 시스템 프롬프트에도 "첫 문장에서 곧바로 명확한 결론부터 제시하고,
2:1로 애매하게 갈리더라도 결론 자체는 흐리지 말 것"을 명시해, 다른 스프레드들과 달리
결과를 모호하게 얼버무리지 않도록 했다. 이걸로 처음 계획했던 12종 스프레드를 전부
완성했다 — 앞으로 새 스프레드가 더 필요해지면 이 파일에 정리된 추가 방식(①`lib/deck.ts`에
...
없고 `cardId`(78장 중 하나)·`orientation`·`style`(12종 화풍 중 하나)·`model`(나노바나나
...
조립한다. 12종 화풍(`CARD_STYLES`: 몽환 수채화·귀여운 캐릭터·한국 전통 동양화·네온
스테인드글라스·다크 아르누보 골드·사이버펑크 네온·코스믹 갤럭시·클래식 유화 초상화·
레트로 픽셀아트·팝아트 코믹·지브리풍 애니메이션·픽사풍 3D 애니메이션)은 `/draw`에서
미리 고른다. 화풍은 스프레드와 마찬가지로 `lib/deck.ts`의 `CARD_STYLES` 객체(코드
상수)로만 관리되고 DB 테이블이 필요 없다 — `Object.keys(CARD_STYLES)`로 자동 순회하는
`DrawFlow.tsx`의 선택 UI 덕분에, 새 항목을 추가하면 별도 화면 수정 없이 바로 선택
목록에 나타난다(2026-09-19, 사용자 요청으로 코스믹 갤럭시/클래식 유화/레트로 픽셀아트/
팝아트 코믹 4종 추가 — 기존 6종이 대부분 "화려하고 장식적인" 계열이라, 신비로운 우주
스케일(코스믹)·사실적 회화(유화)·저해상도 게임 그래픽(픽셀아트)·평면적 만화(팝아트)처럼
느낌이 뚜렷이 다른 화풍 위주로 골랐다).

**지브리풍 애니메이션 / 픽사풍 3D 애니메이션(2026-09-19 추가)**: 사용자가 "일본만화
지브리 스타일과 미국 애니메이션 Pixar 스타일"을 명시적으로 요청했지만, `promptModifier`
(Gemini에게 실제로 보내는 영어 프롬프트)에는 "Studio Ghibli"/"Pixar" 같은 실제 스튜디오
이름을 그대로 넣지 않고 **그 느낌을 만드는 시각적 특징만 서술**했다(손그림 2D 애니메이션 +
수채화 배경 + 목가적 판타지 분위기 / 3D 컴퓨터 애니메이션 + 크고 표현력 있는 눈 + 부드러운
글로벌 일루미네이션). 이 프로젝트가 처음부터 지켜온 "실제 상업 타로 덱을 스캔/모사하지
않고 시각적 특징만 묘사한다"는 원칙과 같은 이유이고, 실무적으로도 일부 이미지 생성
API가 특정 상업 스튜디오명이 포함된 프롬프트를 필터링하거나 거부할 수 있어(2025년
"지브리 스타일" 이미지 생성이 화제가 된 뒤 여러 생성형 AI 서비스가 관련 정책을 강화한
사례가 있다) 생성 자체가 막히는 걸 예방하는 목적도 있다. 화면에 보이는 한글 이름/설명은
사용자가 알아보기 쉽도록 "지브리풍"/"픽사풍"이라는 표현을 그대로 썼다 — "~풍"이라는
표현 자체가 공식 제휴가 아닌 스타일 유사성만을 뜻한다는 게 일반적인 통념이라는 점을
고려한 판단이다.
...
| 23 | 예·아니오(Yes-No) 3카드 스프레드 추가 | ✅ — 계획했던 스프레드 12종 전부 완성 |
| 24 | 코스믹 갤럭시/클래식 유화/레트로 픽셀아트/팝아트 코믹 화풍 4종 추가(총 10종) | ✅ |
| 25 | 지브리풍 애니메이션/픽사풍 3D 애니메이션 화풍 2종 추가(총 12종) | ✅ |
| 26 | `/draw` 화풍 선택(STEP 2) 카드 12종 표시 순서를 사용자 지정 순서로 재배치 | ✅ |
| 27 | `/draw` "카드 뽑기" 버튼 텍스트를 "🔮 {스프레드 제목} ({N}장) 카드 뽑기" 한 줄에서 "🔮 {스프레드 제목}" + 줄바꿈 + "카드 {N}장 뽑기" 2줄 고정 포맷으로 변경 — 스프레드 제목이 길면(예: "관계 심층 분석 (6카드)") 텍스트가 어중간한 위치에서 줄바꿈되던 문제를, 항상 제목/장수 경계에서만 줄바꿈되도록 해결(`whitespace-pre-line` + 명시적 `\n`) | ✅ |

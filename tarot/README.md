...
AIMaster 계정으로 로그인해야 이용할 수 있는 **AI 타로 리딩 웹앱**. 질문을 입력하고 7종
스프레드(원카드/3카드 과거현재미래/3카드 궁합/5카드 심층분석/켈틱크로스 10카드/말굽
7카드/관계 6카드) 중 하나를 골라 카드를 뽑으면,
...
심층분석, 가장 정통적인 10카드 켈틱 크로스, 조언 중심의 7카드 말굽(Horseshoe), 그리고
나와 상대방을 함께 깊게 보는 6카드 관계(Relationship) 스프레드까지 총 7종 스프레드를
추가했다(사용자 요청, 2026-09-19: "실제 타로 세계의 다른 유명 스프레드들도 하나씩
완성해나가보자" — 켈틱 크로스 다음으로 널리 쓰이는 정통 스프레드부터 순서대로 추가하는
중). 각 스프레드는 `positions`(자리 순서)·`positionLabels`(자리별 한글 라벨)·
`positionDescriptions`(자리별 설명)를 자체적으로 갖고 있어, 카드 뽑기(`drawCards()`)와
결과 화면(`ResultInteractive`)이 스프레드 종류에 관계없이 이 설정 하나로 동작한다. 카드
수가 많은 스프레드는 한 번에 해석해야 할 카드가 많아 `/api/generate-reading`의 시스템
프롬프트를 스프레드별로 따로 두고, OpenAI 응답 토큰 한도(`max_completion_tokens`)도
카드 6장 이상이면 2000 → 3000, 10장 이상이면 3500으로 늘려 흐름이 끊기지 않게 했다.
...
**남은 추가 예정 스프레드(순서대로 진행 예정)**: 커리어/진로 스프레드, 한 해 운세(Year
Ahead, 12카드), 생명의 나무(Tree of Life, 10카드), 별/만다라 스프레드, 예/아니오
(Yes-No) 스프레드. 새 스프레드를 추가할 때는 이 파일의 "말굽 스프레드"/"관계 스프레드"
추가 방식(①`lib/deck.ts`에 `SpreadType`/필요한 `SpreadPosition`/`SPREAD_CONFIGS` 항목
추가 ②`app/api/generate-reading/route.ts`에 전용 시스템 프롬프트 추가 ③카드 수가
6장 이상이면 토큰 한도 확인)을 그대로 따르면 된다.
...
| 19 | 커리어/한 해 운세/생명의 나무/별·만다라/예-아니오 스프레드 추가 | ⬜ 남은 작업(순서대로 진행 예정) |

...
AIMaster 계정으로 로그인해야 이용할 수 있는 **AI 타로 리딩 웹앱**. 질문을 입력하고 4종
스프레드(원카드/3카드 과거현재미래/3카드 궁합/5카드 심층분석) 중 하나를 골라 카드를 뽑으면,
...
스프레드 하나만 지원했다(셀틱 크로스 같은 다장 스프레드는 카드 수만큼 해석 로직과 화면
레이아웃이 복잡해져 MVP 범위를 벗어난다고 판단해 제외). 이후 사용성을 넓히기 위해
`lib/deck.ts`의 `SPREAD_CONFIGS`에 부담 없는 원카드(오늘의 운세), 관계를 보는 3카드 궁합,
좀 더 깊은 고민을 위한 5카드 심층분석까지 총 4종 스프레드를 추가했다. 각 스프레드는
`positions`(자리 순서)·`positionLabels`(자리별 한글 라벨)·`positionDescriptions`(자리별
설명)를 자체적으로 갖고 있어, 카드 뽑기(`drawCards()`)와 결과 화면(`ResultInteractive`)이
스프레드 종류에 관계없이 이 설정 하나로 동작한다. 셀틱 크로스 같은 더 큰 다장 스프레드는
여전히 미구현이며, 아래 "남은 작업" 참고.
...
**레거시 공유 링크 복원과 알려진 이슈**: `img`/`imgs`/`rd` 파라미터가 없는 옛날 공유
링크(또는 파라미터 없이 새로 뽑은 경우)를 위해, `app/result/page.tsx`는
`createAdminClient()`(RLS 우회)로 `tarot_readings`에서 **같은 스프레드 + 같은 카드
구성**을 가진 가장 최근 기록을 찾아 이미지·해석을 복원해준다. 이때 `user_id`로 필터링하지
않기 때문에, 특히 원카드 스프레드(78장×2방향=156가지뿐)처럼 카드 조합 가짓수가 적은
스프레드에서는 **전혀 다른 사용자가 우연히 같은 카드를 뽑았을 때 그 사람의 질문·AI
해석이 내 결과에 잘못 복원될 수 있는 구조적 위험**이 있다. 공유 링크로 들어온 경우로
제한하거나 매칭 조건에 `user_id`를 함께 확인하도록 보완이 필요하다(아래 "남은 작업" 참고).

`/api/og`(정적 대체 카드)는 3장을 전부 동적으로 그려 넣는 대신 브랜드 카드(🔮 AI 타로 +
현재 카드 이름만 선택적으로 표시)로 단순화했다 — 실제 생성 이미지가 있으면 위 `img`
파라미터 경로로 그 이미지가 og:image로 우선 쓰이므로, `/api/og`는 "이미지가 아직 없을 때의
대체 카드" 역할만 하면 충분하다고 판단했다. mbti-character와 동일하게 `middleware.ts`의
matcher에서 `api/og` 경로를 로그인 체크 대상에서 제외했다(카카오톡/페이스북 크롤러가
로그인 없이 긁어가야 하므로).

## 남은 작업

- **🔴 [버그 후보] 레거시 공유 링크 복원의 크로스 유저 데이터 노출**: 위 "카카오톡 공유"
  섹션 참고. `/result`가 파라미터 없이 카드 조합만으로 DB를 뒤져 남의 리딩을 복원해줄 수
  있는 구조적 위험이 있다. `user_id` 매칭 추가 또는 공유 링크 여부로 조건 제한 필요.
- **`/history` 삭제 UI**: `tarot_readings`에는 이미 본인 소유 delete RLS 정책이 있는데,
  `components/HistoryList.tsx`에는 개별 리딩을 지우는 버튼이 아직 없다.
- **`/history` 페이지네이션**: `app/history/page.tsx`가 사용자의 전체 리딩 이력을 제한 없이
  한 번에 조회한다. 리딩이 많이 쌓인 헤비 유저에게는 느려질 수 있다.
- **켈틱 크로스 등 대형 스프레드 추가**: 현재 4종(1/3/3/5카드)까지 지원한다. 더 큰 스프레드는
  카드 개수별 레이아웃/해석 프롬프트를 별도로 설계해야 한다.
- **공유 카드에 전체 카드 반영**: 카카오 공유/og:image는 여전히 대표 카드 1장만 이미지로
  쓴다(`imgs` 파라미터로 전체 이미지는 URL에 실어 결과 화면 복원에는 쓰이지만, OG 이미지
  자체는 아직 1장 합성뿐). 여러 장을 합성한 OG 이미지를 만들려면 별도 이미지 합성 로직(예:
  Satori로 카드를 나란히 배치)이 필요하다.
- (실사용자 로그인 테스트 필요) 실제 AIMaster 계정으로 `/login` → `/draw` → `/result` →
  `/history`까지 실기기/브라우저로 한 번 확인 권장.
...
| 11 | 켈틱 크로스 등 대형 스프레드 | ⬜ 남은 작업 |
| 12 | `/history` 삭제 UI + 페이지네이션 | ⬜ 남은 작업 |
| 13 | 레거시 공유 링크 크로스 유저 노출 버그 수정 | ⬜ 남은 작업 |

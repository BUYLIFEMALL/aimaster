...
AIMaster 계정으로 로그인해야 이용할 수 있는 **AI 타로 리딩 웹앱**. 질문을 입력하고 9종
스프레드(원카드/3카드 과거현재미래/3카드 궁합/5카드 심층분석/켈틱크로스 10카드/말굽
7카드/관계 6카드/커리어 6카드/한 해 운세 12카드) 중 하나를 골라 카드를 뽑으면,
...
(Career), 그리고 1월부터 12월까지 한 달에 카드 한 장씩 보는 12카드 한 해 운세(Year
Ahead)까지 총 9종 스프레드를 추가했다(사용자 요청, 2026-09-19: "실제 타로 세계의
다른 유명 스프레드들도 하나씩 완성해나가보자" — 켈틱 크로스 다음으로 널리 쓰이는
정통 스프레드부터 순서대로 추가하는 중). 각 스프레드는 `positions`(자리 순서)·
...
**남은 추가 예정 스프레드(순서대로 진행 예정)**: 생명의 나무(Tree of Life, 10카드),
별/만다라 스프레드, 예/아니오(Yes-No) 스프레드. 새 스프레드를 추가할 때는 이 파일의
"말굽 스프레드"/"관계 스프레드"/"커리어 스프레드"/"한 해 운세 스프레드" 추가 방식
...
| 21 | 생명의 나무/별·만다라/예-아니오 스프레드 추가 | ⬜ 남은 작업(순서대로 진행 예정) |

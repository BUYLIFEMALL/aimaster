...
AIMaster 계정으로 로그인해야 이용할 수 있는 **AI 타로 리딩 웹앱**. 질문을 입력하고
78장 중 3장(과거-현재-미래)을 뽑으면, AI가 그린 카드 일러스트(Gemini)와 AI가 풀어주는
종합 해석(OpenAI)을 결과 화면에서 바로 볼 수 있다. `mbti-character`를 스캐폴드 템플릿으로
삼아 만들었으며, 로그인 필수 구조·API 키 표준 패턴·카카오 공유 컴포넌트를 그대로 가져와
타로 도메인에 맞게 확장했다.
...
### 카드 스프레드는 왜 3장(과거-현재-미래)만 만들었는가
실제 타로 앱들이 가장 널리 쓰는 입문용 스프레드가 과거-현재-미래 3카드 스프레드다.
셀틱 크로스 같은 다장 스프레드는 카드 수만큼 해석 로직과 화면 레이아웃이 복잡해져
MVP 범위를 벗어난다고 판단해 의도적으로 제외했다(mbti-character가 60문항 정식판을
"남은 작업"으로 미룬 것과 같은 방식의 범위 조절) — 아래 "남은 작업" 참고.

### 카드 뽑기 방식
`lib/deck.ts`의 `drawThreeCardSpread()`가 78장(`lib/cards.ts`의 `TAROT_DECK`) 중 서로 다른
3장을 뽑고, 각 카드는 독립적으로 50/50 확률로 정방향/역방향이 결정된다. 결제나 사용자
데이터와 무관한 단순 연출용 랜덤이라 암호학적 난수는 쓰지 않고 브라우저에서 `Math.random`
기반으로 그대로 실행한다(`components/DrawFlow.tsx`). 뽑은 결과는 서버에 저장하지 않고
`cardId:방향` 형태로 직렬화해(`serializeDraw`) `/result?cards=...&q=...` 쿼리스트링으로만
전달한다 — 리딩 이력을 DB에 남기는 기능은 의도적으로 만들지 않았다(아래 "남은 작업" 참고).
...
결과 화면에 진입하면(`components/ResultInteractive.tsx`) mbti-character와 동일하게
"버튼을 눌러야 생성"이 아니라 **자동으로** 뽑힌 카드 3장 각각의 일러스트 생성을 시도한다
(등록된 Gemini 키가 있을 때). `app/api/generate-card-image/route.ts`는
`checkProgramAccessApi()`로 로그인을 확인한 뒤 `resolveApiKey(userId, "gemini")`로 회원
본인 키만 조회해서 쓴다(앱 공용 키 폴백 없음). 클라이언트는 자유 텍스트 프롬프트를 보낼 수
없고 `cardId`(78장 중 하나)와 `orientation`만 보내며, 실제 프롬프트는 서버가
`lib/cards.ts`의 고정 데이터로만 조립한다.
...
`app/api/generate-reading/route.ts`는 뽑힌 카드 3장(카드ID+위치+방향)과 사용자가 입력한
질문(최대 300자, 선택)을 받아 OpenAI(`gpt-4o-mini` — 이 저장소의 다른 서브프로젝트,
특히 `naver-cafe-poster/src/lib/ai/cafeGenerator.ts`가 자유 형식 한국어 텍스트 생성에
공통적으로 쓰는 모델과 동일하게 맞춤)를 호출해 하나로 이어지는 한국어 해석을 생성한다.
시스템 프롬프트에 "과거/현재/미래 자리의 의미", "정방향/역방향의 차이", "확정적 예언·
의학/법률/재정 조언 금지" 같은 실제 타로 리딩 가이드를 담았고, 각 카드의 `lib/cards.ts`
...
"카카오 SDK 공유 버튼 + 항상 링크 복사 버튼" 조합으로 정착된 컴포넌트). 결과 화면은 3장의
카드 각각에 이미지가 생길 수 있지만, 카카오 Feed 템플릿/`og:image`는 이미지 1장만 받을 수
있어 **"현재" 자리 카드의 생성 이미지를 대표 이미지로 사용**하기로 했다(과거/현재/미래 중
지금 이 순간을 보여주는 카드가 가장 대표성이 있다고 판단). 생성되면
`shareUrl = ${shareUrlBase}&img=<현재 카드 이미지 공개 URL>` 형태로 조립해 `ShareButtons`와
`generateMetadata()`(og:image) 양쪽에 넘긴다. `img` 파라미터는 mbti-character와 동일하게
`getTrustedImageUrl()`로 우리 Storage 버킷 URL 접두사인지 검증한 뒤에만 신뢰한다(조작된
`img` 값으로 임의 이미지를 공유 미리보기에 끼워넣는 것을 방지).
...
- **다양한 스프레드(켈틱 크로스 등) 추가**: 현재는 3카드 스프레드만 지원한다. 의도적으로
  미룬 MVP 범위 결정이며, 추가하게 되면 카드 개수별 레이아웃/해석 프롬프트를 별도로
  설계해야 한다.
- **리딩 이력 테이블**: mbti-character도 아직 검사 결과를 서버에 저장하지 않는 것과
  동일하게, 이 프로젝트도 리딩 결과를 DB에 저장하는 기능은 만들지 않았다(뽑힌 카드/질문은
  URL 쿼리스트링으로만 오간다). "내 리딩 히스토리 보기" 같은 기능을 붙이려면
  `user_id` + RLS owner-only 테이블을 새로 추가해야 한다.
- **공유 카드에 3장 전부 반영**: 현재 카카오 공유/og:image는 "현재" 카드 1장만 대표
  이미지로 쓴다. 3장을 합성한 카드 이미지를 만들려면 별도 이미지 합성 로직(예: Satori로
  3장을 나란히 배치한 og 이미지)이 필요하다.
- (실사용자 로그인 테스트 필요) 실제 AIMaster 계정으로 `/login` → `/draw` → `/result`까지
  실기기/브라우저로 한 번 확인 권장. 이 세션에서는 `npm run build` 통과와 미들웨어 리다이렉트
  동작(로그인 없이 `/draw` 접근 시 `/login?redirect=...`)까지만 검증했다.
...
| 3 | 3카드 스프레드 뽑기 + 셔플/뒤집기 연출(`/draw`) | ✅ |
| 4 | 결과 화면: 카드별 AI 일러스트 자동 생성(Gemini, BYOK) | ✅ |
| 5 | 결과 화면: AI 종합 해석 자동 생성(OpenAI, BYOK) | ✅ |

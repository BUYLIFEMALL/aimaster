...
AI가 그린 카드 일러스트(Gemini, 10종 화풍 선택 가능)와 AI가 풀어주는 종합 해석(OpenAI, 모델
...
없고 `cardId`(78장 중 하나)·`orientation`·`style`(10종 화풍 중 하나)·`model`(나노바나나
...
조립한다. 10종 화풍(`CARD_STYLES`: 몽환 수채화·귀여운 캐릭터·한국 전통 동양화·네온
스테인드글라스·다크 아르누보 골드·사이버펑크 네온·코스믹 갤럭시·클래식 유화 초상화·
레트로 픽셀아트·팝아트 코믹)은 `/draw`에서 미리 고른다. 화풍은 스프레드와 마찬가지로
`lib/deck.ts`의 `CARD_STYLES` 객체(코드 상수)로만 관리되고 DB 테이블이 필요 없다 —
`Object.keys(CARD_STYLES)`로 자동 순회하는 `DrawFlow.tsx`의 선택 UI 덕분에, 새 항목을
추가하면 별도 화면 수정 없이 바로 선택 목록에 나타난다(2026-09-19, 사용자 요청으로
코스믹 갤럭시/클래식 유화/레트로 픽셀아트/팝아트 코믹 4종 추가 — 기존 6종이 대부분
"화려하고 장식적인" 계열이라, 신비로운 우주 스케일(코스믹)·사실적 회화(유화)·저해상도
게임 그래픽(픽셀아트)·평면적 만화(팝아트)처럼 느낌이 뚜렷이 다른 화풍 위주로 골랐다).

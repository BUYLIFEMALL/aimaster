...
export default function ProgramsAdminBoard({ programs: initialPrograms, categories: initialCategories, grades }: ProgramsAdminBoardProps) {
  ...
  // 카테고리 자체의 추가/수정/삭제는 페이지 상단의 CategoryManagerButton(모달)에서 처리한다.
  // 그 모달에서 변경 후 router.refresh()를 호출하면 이 컴포넌트는 새 categories prop을
  // 받는데, useState 초기값은 최초 마운트 때만 반영되므로 prop 변경을 여기서 동기화해준다.
  useEffect(() => {

...
export default function StudioPage({ email }: { email: string }) {
  ...
  async function createDraftWithSelectedStrategy() {
    const draft = await prepareDraft();
    if (draft && generateImageWithDraft) await generateImage(draft);
  }
    ...
    <div className="studio-shell">
      ...
      <main className="main">
        ...
        {activeMenu === "new-draft" && <><section className="card new-draft-card" id="new-draft">
          ...
          <button type="button" className="primary" onClick={() => void createDraftWithSelectedStrategy()} disabled={pending || imagePending || !selectedTitle}>{pending ? "초안 생성 중..." : imagePending ? "대표 이미지 생성 중..." : "선택한 전략으로 AI 초안 생성하기"}</button>
        </section>

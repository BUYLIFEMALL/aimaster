  ...
  const results = await chrome.scripting.executeScript({ target: { tabId: tab.id }, args: [title, body], func: async (titleText, bodyText) => {
    ...
    for (let attempt = 0; attempt < 8 && (!titleContainer || !bodyContainer); attempt += 1) {
      for (const doc of documents) {
        titleContainer ||= doc.querySelector(".se-title-text, .se-documentTitle, [class*='se-title'], textarea[placeholder*='제목'], input[placeholder*='제목']");
        bodyContainer ||= [...doc.querySelectorAll('.se-text-paragraph, .se-section-text, .se-module-text, [contenteditable="true"], textarea')].find((element) => !element.closest(".se-documentTitle") && !element.closest(".se-title-text") && !element.closest("[class*='se-title']") && !element.matches("textarea[placeholder*='제목'], input[placeholder*='제목']"));
      }

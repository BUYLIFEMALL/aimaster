...
- [x] 네이버 자동 삽입 기능 확장 (2/2, 태그) — `fillTags()`로 구현. "발행" 버튼은 사람이
      ...
      `#tag-input`에 쉼표로 구분한 태그를 하나씩 사람처럼 입력한다. **다음 확인 필요**:
      실제 데스크톱에서 태그가 정상적으로 입력되는지 사람이 직접 확인.
- [ ] 카테고리 선택 — 발행 설정창 안의 `.option_category__mNwDi`가 현재 카테고리를 보여주는
      것까지만 확인했고, 클릭했을 때 뜨는 드롭다운의 실제 옵션 목록 구조는 아직 조사 전.
      진행하려면: 발행 설정창을 연 뒤 카테고리 표시 영역을 클릭해서 드롭다운을 열고,
      "에디터 구조 분석" 버튼으로 그 상태를 다시 캡처해서 옵션 셀렉터부터 조사해야 한다.

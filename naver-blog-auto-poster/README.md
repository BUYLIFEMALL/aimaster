...
- [x] 이미지 자동 생성 + 클립보드 붙여넣기 (반자동) — 2026-09-21 구현 완료,
      **실사용 검증 대기 중**. 브라우저 보안 정책상 확장이 `<input type="file">`에
      ...
      버튼도 추가함. **다음 확인 필요**: 실제로 클립보드 복사가 되고, 네이버 본문에
      Ctrl+V로 붙여넣기가 실제로 되는지 사람이 직접 확인.

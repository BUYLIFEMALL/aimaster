...
- **본문**: `.se-body` 컨테이너 안에 문단마다 `.se-text-paragraph`(`<p>` 태그, 실제 텍스트는
  ...
  **주의(2026-09-20 실사용 테스트에서 발견한 버그)**: `.se-text-paragraph` 클래스는 본문
  전용이 아니라 **제목 모듈도 내부적으로 재사용**한다. 셀렉터를 `.se-text-paragraph`만
  단독으로 쓰면 DOM 순서상 제목의 문단이 먼저 걸려서, "본문 클릭"이 실제로는 제목을 다시
  클릭하는 사고가 난다(실제로 본문 첫 줄이 제목 뒤에 그대로 붙어버리는 결과 확인함). 반드시
  `.se-body .se-text-paragraph`처럼 본문 컨테이너로 범위를 좁혀서 써야 한다.
...
- **카테고리**: 같은 발행 설정 레이어 안에 `.option_category__mNwDi`가 현재 선택된 카테고리를
  보여준다("카테고리●여행/핫플레이스"). 클릭하면 선택 드롭다운이 열릴 것으로 추정되나, 이번
  캡처에서는 그 드롭다운을 실제로 열어보지 않아서 옵션 목록의 셀렉터는 아직 미확인 — 실제
  자동 입력 코드를 만들 때 추가 조사 필요.
...
- [x] 프로토타입 3 — 제목/본문 자동 입력: `src/lib/humanInput.js`(사람처럼 한 글자씩 타이핑)
      ...
      결과 확인 후 사람이 직접 발행. **다음 확인 필요**: 실제 데스크톱에서 테스트 문구로
      자동 입력이 되는지, 그리고 자연스러운 속도로 들어가는지(너무 빠르거나 로봇처럼 보이지
      않는지) 사람이 직접 확인.
- [ ] 사용자 편집 UI: 생성된 제목/본문/이미지를 자유롭게 수정할 수 있는 화면
- [ ] 네이버 자동 삽입 기능 확장: 이미지 업로드(`filechooser` 이벤트 처리), 태그, 카테고리
      선택까지 포함해서 완성 (현재는 제목/본문만 지원)

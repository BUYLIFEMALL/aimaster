...
- [x] AIMaster 계정 연동("웹 로그인 → 토큰 붙여넣기") — 2026-09-20 구현·검증 완료.
      ...
      연동 아키텍처" 참고.
- [x] AI 생성 파이프라인 (1차, 제목+본문만) — 2026-09-20 구현 완료. 주제 입력 → 제목+본문
      ...
      동일한 "본인 키는 서버에서만 사용" 원칙, CLAUDE.md 멀티테넌시 3번 참고). 아직 셀프
      리뷰 단계와 이미지 자동 생성(Gemini)은 미포함 — **다음 확인 필요**: 실제 데스크톱에서
      주제 입력 → 생성 → 입력창에 채워지는지, 본인 OpenAI 키가 없을 때 안내 메시지가
      제대로 뜨는지 사람이 직접 확인.

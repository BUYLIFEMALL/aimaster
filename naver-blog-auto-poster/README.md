...
- [x] 네이버 자동 삽입 기능 확장 (1/2) — 이미지 업로드: `insertImage()`로 구현.
      ...
      직접 파일을 물어본 뒤 그 경로를 `filechooser` 이벤트에 넘기는 방식. **다음 확인
      필요**: 실제 데스크톱에서 이미지가 정상적으로 본문에 삽입되는지 사람이 직접 확인
      (삽입 완료를 판별하는 정확한 셀렉터는 아직 조사 전이라, 우선 일정 시간 대기 후 사람이
      눈으로 확인하는 방식).
- [ ] 네이버 자동 삽입 기능 확장 (2/2) — 태그, 카테고리 선택 (발행 설정 레이어 안에 있어서
      "발행" 버튼을 먼저 열어야 함 — 위 "프로토타입 2 조사 결과" 참고)

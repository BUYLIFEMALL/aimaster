...
- [ ] 이미지 자동 삽입 — **구조적으로 불가능한 부분이 있음**: 브라우저 보안 정책상
      확장이 `<input type="file">`에 스크립트로 파일을 채워 넣을 수 없다(1단계
      데스크톱 앱은 Playwright의 `filechooser` 이벤트 가로채기라는, 확장에는 없는
      특수 자동화 권한으로 우회했음). 순수 사람 개입 없이는 불가능하므로, 대안으로
      "AI가 생성한 이미지를 클립보드에 복사 → 사용자가 본문에 Ctrl+V로 붙여넣기"
      방식(반자동, 클릭 한 번만 남음)을 검토할 것 — 아직 미착수.
- [x] AI 생성 UI — 2026-09-21 실사용 검증 완료. 1단계 데스크톱 앱과 동일한
      `/api/naver-blog-auto-poster/generate`를 그대로 재사용(본인 OpenAI 키로 서버가
      대신 호출, 1차 초안+2차 셀프 리뷰). 이미지 생성은 이번 단계에서 요청하지
      않음 — 자동 삽입 경로가 없어서(File Input 제약), 클립보드 붙여넣기 기능을
      만들 때 같이 추가할 예정.

...
import {
  ...
  structureInstaCandidates,
  type InstaCandidateDraft,
} from "@/lib/ai/collector";
import type { InstaSourceType } from "@/types/database.types";
...
async function insertCandidates(
  ...
  userId: string,
  sourceType: InstaSourceType,
  sourceInput: string,
  drafts: InstaCandidateDraft[],
) {
  ...
  const now = Date.now();
  const { error } = await supabase.from("insta_candidates").insert(
    drafts.map((d, i) => ({
      user_id: userId,
...
// 후보 하나당 900~1400자를 생성하다 보니(예전 450자보다 훨씬 김), 한 번의 AI 호출에서
// 너무 많이 뽑으면 출력 토큰 한도에 걸릴 수 있어 3건으로 줄였다.
const CATEGORY_PAGE_PICK_COUNT = 3;

/**
 * 방식 1: HTTP — 주어진 URL이 특정 게시글이면 그 글로 게시글 주제 1건을,
 * 여러 게시글이 나열된 카테고리/목록 페이지(예: 네이버 뉴스 섹션)면
 * 그중 무작위 3건을 골라 각각 게시글 주제를 생성한다.
 */
  ...
  try {
    ...
    if (links.length >= CATEGORY_PAGE_MIN_LINKS) {
      ...
      const rawText = valid
        .map((a, i) => `[${i + 1}] ${a.title}\n${a.text}\n출처: ${a.url}`)
        .join("\n\n");
      const drafts = await structureInstaCandidates({ rawText, maxItems: valid.length, apiKey: apiKey ?? "" });
    ...
    const drafts = await structureInstaCandidates({ rawText: text, maxItems: 1, apiKey: apiKey ?? "" });
    await insertCandidates(supabase, user.id, "http", url, drafts);
    revalidatePath("/candidates");
    return { success: true, count: drafts.length };
  } catch (err) {
...
export async function collectFromRssAction(
  ...
  try {
    ...
    const items = await fetchNewsblurStories(sessionCookie, feedId, 3);
    if (items.length === 0) return { error: "가져올 글이 없습니다." };

    const rawText = items
      .map((item, i) => `[${i + 1}] ${item.title}\n${item.text}\n출처: ${item.link}`)
      .join("\n\n");
    const drafts = await structureInstaCandidates({ rawText, maxItems: items.length, apiKey: apiKey ?? "" });
...
export async function collectFromPerplexityAction(
  ...
  try {
    ...
    const drafts = await structureInstaCandidates({ rawText: trendText, maxItems: 3, apiKey: openaiKey ?? "" });
...
export async function deleteCandidateAction(formData: FormData) {
  ...
  await supabase.from("insta_candidates").delete().eq("user_id", user.id).eq("id", id);

...
export async function createSmtpAccountAction(formData: FormData): Promise<CreateSmtpAccountState> {
  ...
  const { error } = await supabase.from("stepmail_smtp_accounts").insert({
    user_id: user.id,
    label,
    provider,
    smtp_host: smtpHost,
...
export async function updateSmtpAccountAction(accountId: string, formData: FormData): Promise<CreateSmtpAccountState> {
  ...
  const { error } = await supabase.from("stepmail_smtp_accounts").update(updatePayload).eq("id", accountId).eq("user_id", user.id);

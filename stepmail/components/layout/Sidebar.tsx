...
// Step Mail은 "리드 업로드 → AI 이메일 작성 → 메일계정 등록 → 예약 발송"까지 순서대로 진행하는
// 단계형 흐름이라, 사이드바도 그 순서를 그대로 위→아래 스텝퍼로 보여준다. API 키 설정은 이
// 흐름의 일부가 아니라 별도 유틸리티라 분리.
const FLOW_STEPS = [
  {
    step: 1,
    href: "/leads",

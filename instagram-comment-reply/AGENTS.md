...
7. **실제 인스타그램 댓글 게시(`postReplyAction`)** — 공개적으로 남는 되돌리기 어려운 행동이다.
   화면에서 사람이 "답변승인" 버튼을 직접 누르거나 텔레그램에서 승인하는 것 자체가 Meta
   Developer Policy §1.7이 요구하는 "사전의 명시적 동의"이므로, 이 흐름을 우회해서 자동/일괄
   게시하는 코드를 추가하지 말 것. **youtube-auto-reply와 달리 (선택) 자동 게시 토글은 Phase 1에
   아예 만들지 않는다** — Meta 정책 §5.2.2(d)가 "정형화되지 않은, 사전 승인된" 메시지만
   허용한다고 더 엄격하게 명시하고 있어 안전 마진을 더 둔다(추가하려면 별도로 신중히 검토).
...
| 2 | Vercel Cron 활성화(사용자 승인 후 — 지금은 vercel.json에 정의만 되어 있고 아직 배포 시 활성화 여부는 별도 확인 필요) | ⏳ 예정 |
| 2 | Webhook 기반 실시간 댓글 수신(현재는 폴링) | ⏳ 예정 |
| 2 | (신중 검토 후) 자동 게시 옵션 | ⏳ 예정 |

...
const IDEOGRAM_MODELS = [
  {
    ...
    input: {
      prompt: "A neon sign in a dark street glowing with text 'BUY LIFE'",
      aspect_ratio: "1:1",
      style_type: "Design",
...
async function testIdeogramEndpoints() {
  console.log("=== Ideogram 모델 엔드포인트 동작 검수 테스트 ===");
  ...
  for (const item of IDEOGRAM_MODELS) {
    console.log(`\n----------------------------------------`);

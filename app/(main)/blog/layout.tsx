...
export default function BlogLayout({ children }: { children: React.ReactNode }) {
  ...
  // /blog(게시글 관리 홈), /blog/write/ai-form(AI 글쓰기), /blog/candidates(게시글 주제 수집)는
  // 사이드바(BlogSidebar)를 갖는 라우트이며, 나머지 블로그 하위 페이지(/blog/posts/[id] 등)와
  // 동일한 화이트 라이트 테마("AutoBlog" 톤)를 쓴다. 사이드바 바깥의 루트 헤더/푸터(다크골드)는
  // 사이트 전체 공통 프레임으로 그대로 유지된다.
  const showSidebar =
    pathname === '/blog' || pathname.startsWith('/blog/write') || pathname.startsWith('/blog/candidates')

...
// 이 앱(longtail-keyword-expander)은 AIMaster와 같은 Supabase 프로젝트를 공유한다.
// 로그인 여부만으로는 부족하고, AIMaster의 programs/subscriptions/user_program_access
// 테이블을 기준으로 "이 프로그램을 실제로 이용할 권한이 있는지"까지 확인해야 한다.
// stepmail/crm-google-form/booking-reminder와 동일한 패턴.
type SupabaseLike = {
...
> {
  ...
  return { allowed: false, error: "롱테일 키워드분석 자동화 이용 권한이 없습니다. 구독 후 이용해주세요.", status: 403 };
}

/**
 * 실제 비용이 발생하는 동작 1건을 usage_logs에 기록한다.
 * RLS insert 정책상 service role(admin client)로만 기록 가능하다.
 * 텔레메트리 성격이므로 기록 실패가 본 기능(분석 등)을 막아서는 안 된다.
 */

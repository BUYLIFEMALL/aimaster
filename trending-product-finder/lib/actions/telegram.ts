...
// program_slug) 단위로 스코프된다 — 다른 프로그램에서 이미 텔레그램을 연결한 회원이라도,
// 이 프로그램에서는 별도로 연동해야 한다.
const THIS_PROGRAM_SLUG = "trending-product-finder";

export async function connectTelegramAction(_prevState: TelegramActionState, formData: FormData): Promise<TelegramActionState> {
  ...
  if (!chatInfo) {
    return {
      error: "아직 봇에게 보낸 메시지를 찾지 못했어요. 텔레그램에서 본인 봇을 열고 아무 메시지나 먼저 보낸 뒤 다시 시도해주세요.",
    };
  }
  ...
  try {
    await sendTelegramMessage({
      ...
      text: "✅ 상품소싱 자동화 알림 연동이 완료됐어요. 앞으로 리포트가 자동 생성되면 여기로도 요약을 알려드릴게요.",
    });
  } catch {

...
export function SolapiAccountSection({ account }: { account: SolapiAccountData | null }) {
  ...
  async function handleDelete() {
    if (!confirm("SOLAPI 계정을 삭제할까요? 카카오톡 리포트 알림이 중단됩니다.")) return;
    setIsDeleting(true);
    try {
  ...
  return (
    <div className="space-y-3 rounded-2xl border-2 border-gray-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        {account && !isEditing && (
          <button type="button" onClick={() => setIsEditing(true)} className="text-xs font-bold text-sky-600 hover:underline">
            ✏️ 수정
          </button>
        )}
      </div>
      <p className="text-xs text-gray-500">
        <a href="https://solapi.com/" target="_blank" rel="noreferrer" className="font-semibold text-sky-600 hover:underline">
          solapi.com
        </a>
        에서 발급받은 API Key/Secret과, SOLAPI에 등록·인증된 발신번호를 등록하세요. 카카오
        친구톡으로 리포트를 받으려면 카카오 비즈니스 채널 ID(pfId)도 함께 등록해야 합니다
        (SMS 테스트만 쓸 거면 비워둬도 됩니다).
      </p>
      ...
      {account && !isEditing ? (
        <div className="space-y-3 rounded-xl bg-gray-50 p-4">
          ...
          <p className="text-sm text-gray-700">
            카카오 채널(pfId): {account.kakao_pf_id ?? <span className="text-gray-400">미등록 — 리포트 카카오톡 알림 안 됨</span>}
          </p>
          ...
          <div className="flex flex-wrap items-center gap-3 border-t border-gray-200 pt-3">
            <button
              ...
              className="text-xs font-semibold text-sky-600 hover:underline disabled:opacity-60"
            >
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-gray-200 pt-3">
            <input
              ...
              className="input-sm min-w-[200px] flex-1"
            />
            <button
              type="button"
        ...
        <form onSubmit={handleSave} className="space-y-3 rounded-xl bg-gray-50 p-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input name="apiKey" required defaultValue={account?.api_key ?? ""} autoComplete="off" className="input-sm w-full" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-700">API Secret</label>
              <input
                name="apiSecret"
                type="password"
                required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-700">발신번호</label>
              <input name="senderPhone" required defaultValue={account?.sender_phone ?? ""} placeholder="01012345678" className="input-sm w-full" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-700">카카오 채널 ID (pfId, 선택)</label>
              <input name="kakaoPfId" defaultValue={account?.kakao_pf_id ?? ""} placeholder="KA01PF..." className="input-sm w-full" />
            </div>
          </div>

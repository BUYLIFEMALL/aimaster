  ...
  return (
    <div className="space-y-4 bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-relaxed text-amber-800">
        ...
        <div className="mt-2">
          <p>
            <span className="font-bold">1. 띄어쓰기 금지</span> — 네이버 검색광고 API는 띄어쓰기가
            하나라도 들어가면 요청 자체를 거부합니다.
          </p>
          <p className="mt-1">
            &quot;캠핑 의자&quot;처럼 띄어쓰기가 있으면 저희가 자동으로 &quot;캠핑의자&quot;로 붙여서
            검색합니다.
          </p>
          <p className="mt-1">(검색 결과 상단에 실제로 검색된 키워드를 보여드리니 참고해주세요.)</p>
        </div>

        <p className="mt-2">
          <span className="font-bold">2. 특수문자 주의</span> — 특수문자(!@#$% 등)가 섞이면 거부될 수
          있으니, 가능하면 순수 한글/영문 단어로 입력해주세요.
        </p>

        <div className="mt-2">
          <p>
            <span className="font-bold">3. 호출 한도 주의</span> — 네이버 API 호출 한도(초당 몇 회)
            때문에 짧은 시간에 여러 번 연속으로 누르면
          </p>
          <p className="mt-1">
            <span className="font-bold">&quot;호출 한도를 초과했습니다&quot;</span> 에러가 날 수
            있습니다.
          </p>
          <p className="mt-1">이 경우 몇 초 후 다시 시도해주세요.</p>
        </div>
      </div>

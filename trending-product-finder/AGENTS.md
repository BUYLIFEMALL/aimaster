...
| 5 | 도매매(dome.co.kr) Open API를 `/sourcing`에 알리익스프레스와 나란히 추가 — 국내 위탁소싱이라 관세/부가세/해외운송비 계산 불필요, 개인 ID 로그인만으로 API Key 즉시 무료 발급 가능해 BYOK 원칙에 가장 적합 | ✅ 구현 완료(2026-09-01) — `lib/domeggook/client.ts`(상품리스트 API, market=supply), `/sourcing`에 알리익스프레스/도매매 채널 토글 UI 추가. **실계정 미검증**: 공식 문서만으로 구현해서 응답 JSON의 정확한 필드명(특히 root 키가 `domeggook`인지)은 실사용 시 재확인 필요 |

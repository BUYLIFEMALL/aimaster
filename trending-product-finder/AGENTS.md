...
| 8 | 11번가 상품검색 API로 "경쟁 상품 수·평균가" 지표를 트렌드 리포트에 결합(쿠팡파트너스 승인 없이도 경쟁도 지표 부활) | ⏸️ 대기 — **1순위, 다음 착수 후보**(2026-09-02 백로그 등록). 신규 API 발급 불필요, Phase 7에서 이미 연동한 `lib/elevenst/client.ts` 재사용 |
| 9 | 소싱 후보 카드에 "이 상품으로 쓰레드 홍보글 만들기" 버튼 → `threads-affiliate-poster`로 원클릭 연결 | ⏸️ 대기 — **1순위**(2026-09-02 백로그 등록). 신규 API 불필요, 두 서브프로젝트 모두 완성 상태라 연결 로직만 추가하면 됨 |
...
| 11 | 관심 키워드 전체를 한 번에 마진계산 → 마진율 순 정렬 + CSV 내보내기 | ⏸️ 대기 — **1순위**(2026-09-02 백로그 등록). 신규 API 불필요, 순수 UX/배치처리 개선 |

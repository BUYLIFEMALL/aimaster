...
| 10 | 매일 자동 생성된 리포트를 이메일로 요약 발송(로그인 없이도 확인) | ⏸️ 대기 — **1순위**(2026-09-02 백로그 등록). 루트 AIMaster의 운영자 SMTP(`lib/email/*`, 가입환영/구독만료 메일과 동일)를 재사용 — 회원별 API 키가 필요한 "엔진" 자동화가 아니라 플랫폼 알림이라 BYOK 원칙과 무관 |
| 11 | 관심 키워드 전체를 한 번에 마진계산 → 마진율 순 정렬 + CSV 내보내기 | ✅ 구현 완료, 배포됨(2026-09-02) — `/sourcing`에 `BatchMarginCalculator` 추가. 관심 키워드(카테고리별 그룹) 체크박스 선택 + 직접 입력 병행, 선택 채널마다 검색해 **최저가 상품을 대표값**으로 `calcMargin()`(플랫폼별 기본값 재사용)을 돌려 마진율 순 정렬 테이블로 표시, CSV(UTF-8 BOM, Excel 한글 호환) 다운로드 지원. 최대 20개 키워드 제한. 서버 액션(`lib/actions/batchMargin.ts`)은 기존에 각각 실계정 검증된 3개 플랫폼 클라이언트(`aliexpress`/`domeggook`/`elevenst`)를 그대로 재사용하는 순수 오케스트레이션이라 로직 재검증 위험은 낮으나, **버튼 클릭→CSV 다운로드까지의 브라우저 실사용 E2E는 아직 미확인** — 실계정(buylifemall)에 관심 키워드 7개(4개 카테고리)가 등록돼 있어 화면에 정상 노출될 것으로 예상되나 직접 클릭 테스트 필요 |

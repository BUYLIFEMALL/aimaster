...
| 10 | 매일 자동 생성된 리포트를 이메일로 요약 발송(로그인 없이도 확인) | 🟡 코드 구현 완료·배포됨(2026-09-02), **실제 발송은 SMTP 환경변수 등록 전까지 비활성** — `lib/email/client.ts`(루트 `lib/email/client.ts`와 동일 패턴의 nodemailer 발신기, 이 프로젝트는 별도 배포라 독립 구현), `lib/email/reportSummary.ts`(기회점수 상위 5개 요약 HTML). `lib/reportEngine.ts`에 `options.notifyEmail` 플래그 추가 — **cron 자동 생성분만** 이메일을 보내고 "지금 리포트 생성" 버튼(수동)은 이미 화면을 보고 있으니 중복 알림을 피하려 보내지 않음. 회원별 API 키가 필요한 "엔진" 자동화가 아니라 운영자가 전 회원에게 보내는 플랫폼 알림이라 BYOK 원칙과 무관, 루트의 가입환영/구독만료 메일과 같은 운영자 SMTP 계정을 재사용하면 됨. **이 프로젝트는 루트와 별개의 Vercel 배포라 SMTP_HOST/SMTP_PORT/SMTP_USER/SMTP_PASSWORD/EMAIL_FROM을 여기에도 (루트와 같은 값으로) 등록해야 실제 발송됨** — 미등록 시 콘솔 경고만 남기고 조용히 건너뜀(에러 아님). SMTP 비밀번호는 API 키/환경변수라 사용자 승인 없이 임의로 다루지 않음(2026-09-02 자율성 지침) — 등록 전까지 사용자에게 안내만 하고 대기 |

...
| 7 | 11번가 오픈API로 국내 오픈마켓 상품검색 추가(쿠팡 승인 대기 중 대안) | ✅ 구현 완료(2026-09-02) — `lib/elevenst/client.ts`(apiCode=ProductSearch, XML/EUC-KR 응답을 `<ProductCode>` 위치 기준으로 방어적 파싱), `/sourcing`에 알리익스프레스/도매매/11번가 3채널 체크박스 UI로 확장. **재조사 결과 정정**: 기존에 "개인셀러 전환 필요"로 기록했던 것과 달리, 셀러 등록 없이 "서비스 등록"(개인 회원가입만으로 가능, 사업자등록 불필요)만으로 발급되는 일반 Open API 등급으로 확인됨(셀러 API는 상품등록/주문관리 전용, ProductSearch는 그와 별개인 조회 전용 낮은 권한). API 키 유효기간 180일 정책 있음(설정 페이지에 안내). **실계정 미검증**: 발급 절차상 이번 세션에서 직접 키를 받아 테스트하지 못함 — 응답 XML의 정확한 wrapper 태그/필드명은 공식 문서 기준 추정이라, 회원이 키를 등록하고 첫 검색을 해볼 때 재확인 필요 |

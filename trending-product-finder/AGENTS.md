...
| 10 | 매일 자동 생성된 리포트를 이메일로 요약 발송(로그인 없이도 확인) | ✅ 구현 완료, 배포됨(2026-09-02) — **최초엔 운영자 공용 SMTP(env var)로 설계했다가, "설정 페이지에서 각 회원이 본인 값을 넣게 해야지"라는 사용자 피드백으로 BYOK 방식으로 다시 구현함.** 공용 `user_smtp_accounts` 테이블(stepmail/crm-google-form/booking-reminder가 이미 만들어둔 것, 프로그램 접두어 없음)을 마이그레이션 없이 그대로 재사용 — 회원이 다른 프로그램(STEP Mail 등)에서 이미 SMTP 계정을 등록했다면 이 프로젝트 설정 페이지에도 자동으로 보이고 바로 재사용됨(실계정 buylifemall에 이미 gmail/naver 2개 계정이 있어 즉시 확인됨). `lib/constants.ts`(SMTP_PROVIDER_PRESETS), `lib/email/transport.ts`(sendViaSmtpAccount, booking-reminder와 동일 패턴), `lib/actions/smtpAccounts.ts`(등록/수정/삭제/테스트발송, stepmail 패턴 재사용), `components/settings/{SmtpAccountForm,SmtpAccountCard,ProviderAccountSection}.tsx`(디자인만 이 프로젝트 sky 팔레트에 맞게 조정), 설정 페이지에 "📧 리포트 이메일 알림" 박스 추가. `lib/reportEngine.ts`에 `options.notifyEmail` 플래그 — **cron 자동 생성분만** 이메일을 보내고(회원의 `is_active=true`인 SMTP 계정 1개를 조회해서 발송) "지금 리포트 생성" 버튼(수동)은 이미 화면을 보고 있으니 중복 알림이라 보내지 않음. 계정 미등록 시 조용히 건너뜀(에러 아님). 운영자 SMTP 방식 때 만들었던 `lib/email/client.ts`는 삭제함. **미검증**: 버튼 클릭(테스트 발송)부터 cron 자동 발송까지 브라우저 실사용 E2E는 아직 안 함 |
...
| 14 | 저장된 소싱 후보(알리/도매매/11번가)를 크론으로 주기 재조회해 품절·가격인상 감지 알림 | ⏸️ 대기 — 2순위(2026-09-02 백로그 등록). Phase 4a 크론 인프라 재사용 가능 |

...
export default async function HistoryPage() {
  ...
  const [{ data: posted }, { count: postedTotal }, { count: postedThisWeek }, { count: pendingCount }] =
    await Promise.all([
      supabase
        ...
        .select("id, video_id, author_display_name, comment_text, generated_reply, replied_at")
        .eq("user_id", user.id)
        .eq("status", "posted")
  ...
  const videoIds = Array.from(new Set((posted ?? []).map((c) => c.video_id)));
          ...
          {posted.map((c) => (
            <div key={c.id} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
              <div className="mb-2 flex items-center justify-between">
                <p className="text-xs font-semibold text-gray-400">🎬 {titleByVideoId.get(c.video_id) ?? "(삭제된 영상)"}</p>
                {c.replied_at && <p className="text-xs text-gray-400">{formatDateTimeKo(c.replied_at)}</p>}
              </div>
              <p className="text-xs text-gray-400">{c.author_display_name ?? "익명"}의 댓글</p>

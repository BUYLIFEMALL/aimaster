...
// stepmail/real_estate_sales/threads/crm-google-form의 CRON_SECRET Bearer 인증 패턴을
...
/**
 * 연결된 모든 유튜브 채널에 대해 access_token 갱신을 시도해서 실제로 살아있는지 점검한다.
...
export async function GET(request: NextRequest) {
  ...
  const { data: accounts, error } = await admin
    ...
    .select("user_id, channel_title, access_token, refresh_token, token_expires_at, reconnect_notified_at");
  if (error) {
  ...
  for (const account of accounts ?? []) {
    checked += 1;
    const clientId = await getUserApiKey(admin, account.user_id, "google_client_id");
    const clientSecret = await getUserApiKey(admin, account.user_id, "google_client_secret");
    if (!clientId || !clientSecret) continue; // 키를 지운 사용자는 점검할 수 없으니 건너뜀

    try {
      await getValidYoutubeAccessToken(admin, account.user_id, account, clientId, clientSecret);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      if (message !== "YOUTUBE_RECONNECT_REQUIRED") continue; // 일시적 오류는 다음 점검에서 재시도
      newlyBroken += 1;
